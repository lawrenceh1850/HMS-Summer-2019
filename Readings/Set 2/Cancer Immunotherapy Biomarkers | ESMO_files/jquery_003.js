...
(function($, window, document, undefined) {
    $.fn.socialfeed = function(_options) {
        ...
        function SocialFeedPost(social_network, data) {
            ...
            if ( data.social_network != 'rss') {
                this.content.text = Utility.wrapLinks(Utility.shorten(data.message + ' ' + data.description), data.social_network);
            } else {
                this.content.text = Utility.wrapLinks(Utility.shorten(data.message), data.social_network);
            }

            this.content.moderation_passed = (options.moderation) ? options.moderation(this.content) : true;
            this.content.retweeted_account = data.retweeted_account;
            this.content.retweeted_account_name = data.retweeted_account_name;
            this.content.retweeted_account_link = data.retweeted_account_link;
                ...
                utility: {
                    ...
                    unifyPostData: function(element) {
                        ...
                        if (element.id) {
                            ...
                            //change message text if post is a retweet
                            if (element.retweeted_status && element.retweeted_status.text.length > 0) {
                                var retweeted_text = element.text.split(":");
                                if (retweeted_text) {
                                    post.retweeted_account = element.entities.user_mentions[0].screen_name;
                                    post.retweeted_account_name = element.entities.user_mentions[0].name;
                                    post.retweeted_account_link = 'http://twitter.com/' + element.entities.user_mentions[0].screen_name;
                                }
                                post.message = element.retweeted_status.text;
                            }else{
            ...
            rss : {
                ...
                getData: function(url) {
                    var count       = options.rss.limit,
                    	api_key     = 'v3rn9brhqfncl0phytgp9tchs37p8prsdubumfzi'
                        request_url = Feed.rss.api + "&api_key=v3rn9brhqfncl0phytgp9tchs37p8prsdubumfzi&count=" + count + "&rss_url=" + encodeURIComponent(url),
                ...
                utility: {
                    ...
                    unifyPostData: function(index, element){
                        ...
                        post.id = '"' + item.id + '"';
                        post.dt_create= moment(item.pubDate, 'YYYY-MM-DDTHH:mm:ssZ', 'en');
                        ...
                        if ( $.isArray( item.title ) && item.title[0] ) {
                            post.message = item.title[0];
                        } else {
